...
export default function InfoMapData(props) {
	...
	const aboutPojectRef = useRef(null);
	...
	useEffect(() => {
		axiosConfig
			...
			.then(function (response) {
				...
				const infoMapData = response.data.error
					? null
					: response.data.data;
				infoMapData === null
					? setInitialValues({
					...
					: setInitialValues({
							info_id: infoMapData.id,
							id: props.params.id,
							heading: infoMapData.propertyName,
							subheading: infoMapData.propertyLocation,
							description: infoMapData.propertyDescription,
							about_project: infoMapData.aboutProject,
							build_year: infoMapData.buildDetails.buildYear,
							build_area: infoMapData.buildDetails.buildUpArea,
							carpet_area: infoMapData.buildDetails.carpetArea,
							property_type:
								infoMapData.buildDetails.propertyType,
							booking_type: infoMapData.buildDetails.bookingType,
							min_price: infoMapData.buildDetails.price.min_price,
							max_price: infoMapData.buildDetails.price.max_price,
							sqft_price: infoMapData.sqft_price,
							banner: infoMapData.bannerImages,
					...
					<div className="mx-auto">
						<Formik
							...
							onSubmit={(values, { setSubmitting }) => {
								...
								axiosConfig
									...
									.then(function (response) {
										...
										navigate(
											`/dashboard/listings/create/${props.params.id}/actual-site`
										);
									})
									.catch(function (error) {
										console.log(error);
							...
							validationSchema={Yup.object().shape({
								...
								build_year: Yup.string().required(
									"Category is required"
								),
								build_area:
									Yup.string().required("Type is required"),
								carpet_area: Yup.string().required(
									"Carpert Area is required"
								),
								...
								return (
									...
									>
										...
										<div className="w-full flex flex-col gap-2 mt-4">
											...
											<Editor
												onInit={(evt, editor) =>
													(aboutPojectRef.current =
														editor)
												}
												initialValue={
													values.about_project
												}
												init={{
													height: 300,
													menubar: false,
															...
															(image) => (
																...
																>
																	...
																	<span
																		...
																		className="bg-white absolute top-2 right-2 p-1 rounded-full"
																	>
																		<svg
										...
										<div className="flex mt-6">
											<div className="max-w-2xl rounded-lg ">
												<div className="my-4">
													<label className="inline-block mb-2 text-primary text-xs font-semibold">
														Add banner Image
														{errors.banner &&
															touched.banner && (
																<div className="input-feedback text-red-400 text-xs font-semibold capitalize">
										...
										<div className="mt-5 flex item-center space-x-4">
											<button
												...
												className={`capitalize border-box h-11 text-sm px-10 bg-dashboardBlue text-white font-bold hover:opacity-80 hover:text-white transition duration-200 ease-in-out rounded-full ${
													isSubmitting
														? "opacity-20"
														: ""
												}`}
											...
											{/* <Link
												...
												className={`py-3 uppercase border-box h-11 text-sm px-10 bg-blue-400 text-white font-bold hover:opacity-80 hover:text-white transition duration-200 ease-in-out rounded-full`}
											>
												Continue
											</Link> */}
											{/* <Link
												to={`/dashboard/listings/create/${values.id}`}
												className={`py-3 uppercase border-box h-11 text-sm px-10 bg-primary text-white font-bold hover:opacity-80 hover:text-white transition duration-200 ease-in-out rounded-full`}
											>
												Back
											</Link> */}
										</div>
